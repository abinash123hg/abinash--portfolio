    ...
    <>
      ...
      >
        <div className="flex min-w-0 items-center gap-2">
          <div className="relative">
            ...
            {appleMenuOpen && (
              ...
              >
                <button onClick={() => { sound.tap(); openDesktopWindow('about'); setAppleMenuOpen(false); }} className="w-full text-left px-2.5 py-1 rounded-md hover:bg-[#007aff] hover:text-white transition-colors cursor-pointer font-medium">About This Portfolio (Abinash)</button>
                <button onClick={() => { sound.tap(); openDesktopWindow('settings'); setAppleMenuOpen(false); }} className="w-full text-left px-2.5 py-1 rounded-md hover:bg-[#007aff] hover:text-white transition-colors cursor-pointer flex items-center justify-between"><span>System Settings...</span><span className="text-[10px] opacity-50 font-mono">\u2318,</span></button>
                <div className="h-px bg-black/10 dark:bg-white/10 my-1" />
                <button onClick={() => { sound.tap(); openDesktopWindow('recruiter'); setAppleMenuOpen(false); }} className="w-full text-left px-2.5 py-1 rounded-md hover:bg-[#007aff] hover:text-white transition-colors cursor-pointer flex items-center justify-between"><span>Recruiter Dossier & Brief</span><Sparkles className="w-3 h-3 text-amber-400" /></button>
                <button onClick={() => { sound.tap(); openDesktopWindow('quiz'); setAppleMenuOpen(false); }} className="w-full text-left px-2.5 py-1 rounded-md hover:bg-[#007aff] hover:text-white transition-colors cursor-pointer flex items-center justify-between"><span>AI & ML Knowledge Quiz</span><Brain className="w-3 h-3 text-cyan-400" /></button>
                <div className="h-px bg-black/10 dark:bg-white/10 my-1" />
                <button onClick={() => { sound.tap(); closeAllDesktopWindows(); setAppleMenuOpen(false); }} className="w-full text-left px-2.5 py-1 rounded-md hover:bg-[#007aff] hover:text-white transition-colors cursor-pointer text-red-500 hover:text-white">Close All Windows & Sleep</button>
                <div className="h-px bg-black/10 dark:bg-white/10 my-1" />
                <div className="px-2.5 py-0.5 text-[10px] text-neutral-400 font-mono flex items-center justify-between"><span>macOS Sequoia 15.4</span><span>Darwin x86_64</span></div>
              </div>
            )}
          </div>
        ...
        <div className="flex shrink-0 items-center gap-3.5 text-[12px] text-neutral-800 dark:text-neutral-200">
          ...
          <button
            onClick={() => { sound.tap(); onOpenSpotlight(); }}

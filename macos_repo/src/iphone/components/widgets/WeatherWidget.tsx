...
export const WeatherWidget: React.FC<WeatherWidgetProps> = ({ size }) => {
      ...
      <div className="w-full h-full p-4 flex flex-col justify-between select-none">
        <div className="flex items-start justify-between">
          <div>
            ...
            <div className={`text-[10px] ${muted}`}>Mostly Clear • Air Quality 48 (Good)</div>
          </div>
        ...
        <div className={`grid grid-cols-4 gap-2 text-center py-2 border-y my-1 ${isDark ? 'border-white/10' : 'border-zinc-200'}`}>
          <div>
            <span className={`text-[9px] block ${muted}`}>Now</span>
            <Sun className="w-3.5 h-3.5 mx-auto text-amber-500 my-0.5" />
            <span className={`text-[10px] font-semibold ${primary}`}>29°</span>
          </div>
          <div>
            <span className={`text-[9px] block ${muted}`}>1 PM</span>
            <Sun className="w-3.5 h-3.5 mx-auto text-amber-500 my-0.5" />
            <span className={`text-[10px] font-semibold ${primary}`}>31°</span>
          </div>
          <div>
            <span className={`text-[9px] block ${muted}`}>4 PM</span>
            <CloudSun className="w-3.5 h-3.5 mx-auto text-sky-500 my-0.5" />
            <span className={`text-[10px] font-semibold ${primary}`}>28°</span>
          </div>
          <div>
            <span className={`text-[9px] block ${muted}`}>7 PM</span>
            <CloudSun className="w-3.5 h-3.5 mx-auto text-indigo-500 my-0.5" />
            <span className={`text-[10px] font-semibold ${primary}`}>25°</span>
          </div>
        </div>
  ...
  return (
    <div className="w-full h-full p-3.5 flex flex-col justify-between select-none">
      <div className="flex items-start justify-between">
        <div>
          <span className={`text-xs font-bold block ${primary}`}>Bhubaneswar</span>
          <span className={`text-[10px] ${muted}`}>Clear</span>
        </div>

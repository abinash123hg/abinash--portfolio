    ...
    <AnimatePresence>
      ...
      >
        ...
        >
          ...
          <div className="flex items-center bg-zinc-900/80 border border-white/15 rounded-full p-0.5 shadow-md">
            {PAGES.map((page) => {
          ...
          <button
            ...
            className="w-8 h-8 rounded-full bg-white/10 hover:bg-white/20 text-white flex items-center justify-center border border-white/10 active:scale-90 transition-all"
        ...
        <div
          className="absolute right-2.5 top-1/2 -translate-y-1/2 z-30 flex flex-col items-center gap-2 p-1.5 rounded-full bg-black/40 border border-white/10 backdrop-blur-md"
          onClick={(e) => e.stopPropagation()}
        >
          {PAGES.map((page) => {
            const isSelected = controlCenterPage === page.id;
            return (
              <button
                ...
                className={`w-2 rounded-full transition-all duration-300 ${
                  isSelected ? 'h-6 bg-white shadow-sm' : 'h-2 bg-white/30 hover:bg-white/50'
                }`}
                title={page.label}
              />
            );
        ...
        <AnimatePresence>
          {activeToast && (
            <motion.div
              ...
              animate={{ y: 0, opacity: 1 }}
              exit={{ y: -50, opacity: 0 }}
              className="absolute top-14 left-4 right-4 z-50 p-3 rounded-2xl bg-zinc-900/95 border border-white/20 shadow-2xl backdrop-blur-2xl text-white flex items-center gap-3"
            >
